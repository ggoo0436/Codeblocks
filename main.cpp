...
namespace crack{
    ...
    ByteData * zlib_uncompress(ByteData * compressed_data){
        char * tmp_data = (char *) malloc(sizeof(char) * compressed_data->max_len*10);
    ...
    if(argc == 1){
        ...
        for(int i = 1 ; i < argc; i++){
            if( (DocType) type_tmp == LOCAL){
                if( (Mode) mode_tmp == TO_XML) {

                        hackIntoXml(argv[i]);
                        compress_times = true_compress_times;
                        hackIntoXml(argv[i]);
                        compress_times = 10;
                        true_compress_times = 10;
